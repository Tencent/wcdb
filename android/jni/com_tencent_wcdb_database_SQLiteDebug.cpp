...
namespace wcdb {
    ...
    static void nativeGetIOTraceStats(JNIEnv *env, jclass cls, jlong connectionPtr, jobject statsList) {
        struct {
            jfieldID dbName;
            jfieldID path;
            jfieldID pageSize;
        ...
        if (!clsArrayList) return;
        jmethodID midArrayListAdd = env->GetMethodID(clsArrayList, "add", "(Ljava/lang/Object;)Z");
        if (!midArrayListAdd) return;

        jclass clsIOTraceStats = env->FindClass("com/tencent/wcdb/database/SQLiteDebug$IOTraceStats");
        if (!clsIOTraceStats) return;
        jmethodID midIOTraceStatsCtor = env->GetMethodID(clsIOTraceStats, "<init>", "()V");
        if (!midIOTraceStatsCtor) return;
...
#undef GET_FID
        ...
        if (rc != SQLITE_OK) {
            ...
            char sql[256];
            sqlite3_snprintf(sizeof(sql), sql, "SELECT 0 as seq, 'main' as name, %Q as file;", path);
        ...
        while ((rc = sqlite3_step(stmt)) == SQLITE_ROW) {
            ...
            if (sqlite3_prepare_v2(db, sql, -1, &stmt2, NULL) == SQLITE_OK) {
                if (sqlite3_step(stmt2) == SQLITE_ROW) {
                    const char *journalMode = (const char *) sqlite3_column_text(stmt2, 0);
                    jstring journalModeStr = env->NewStringUTF(journalMode);
                    env->SetObjectField(statsObj, fieldsStats.journalMode, journalModeStr);
            ...
            if (rc == SQLITE_OK) {
                ...
                if (f) {
                    ...
                    if (stats.lastMainReadOffset >= 0) {
                        ...
                        f->pMethods->xRead(f, page, pageSize, stats.lastMainReadOffset);
                        env->ReleaseByteArrayElements(pageArr, page, 0);
                        env->SetObjectField(statsObj, fieldsStats.lastReadPage, pageArr);
                        env->DeleteLocalRef(pageArr);
                    } else {
                        env->SetObjectField(statsObj, fieldsStats.lastReadPage, NULL);
                    }
                    ...
                    if (stats.lastMainWriteOffset >= 0) {
                        ...
                        f->pMethods->xRead(f, page, pageSize, stats.lastMainWriteOffset);
                        env->ReleaseByteArrayElements(pageArr, page, 0);
                        env->SetObjectField(statsObj, fieldsStats.lastWritePage, pageArr);
                        env->DeleteLocalRef(pageArr);
                    } else {
                        env->SetObjectField(statsObj, fieldsStats.lastWritePage, NULL);
                    }
                }
                ...
                if (f) {
                    ...
                    if (stats.lastJournalReadOffset >= 0) {
                        ...
                        jbyte *page = env->GetByteArrayElements(pageArr, NULL);
                        f->pMethods->xRead(f, page, pageSize, stats.lastJournalReadOffset);
                        env->ReleaseByteArrayElements(pageArr, page, 0);
                        env->SetObjectField(statsObj, fieldsStats.lastJournalReadPage, pageArr);
                        env->DeleteLocalRef(pageArr);
                    } else {
                        env->SetObjectField(statsObj, fieldsStats.lastJournalReadPage, NULL);
                    }
                    ...
                    if (stats.lastJournalWriteOffset >= 0) {
                        ...
                        page = env->GetByteArrayElements(pageArr, NULL);
                        f->pMethods->xRead(f, page, pageSize, stats.lastJournalWriteOffset);
                        env->ReleaseByteArrayElements(pageArr, page, 0);
                        env->SetObjectField(statsObj, fieldsStats.lastJournalWritePage, pageArr);
                        env->DeleteLocalRef(pageArr);
                    } else {
                        env->SetObjectField(statsObj, fieldsStats.lastJournalWritePage, NULL);
                    }
                }

                env->SetLongField(statsObj, fieldsStats.lastReadOffset, stats.lastMainReadOffset);
                env->SetLongField(statsObj, fieldsStats.lastWriteOffset, stats.lastMainWriteOffset);
                env->SetLongField(statsObj, fieldsStats.lastJournalReadOffset, stats.lastJournalReadOffset);
                env->SetLongField(statsObj, fieldsStats.lastJournalWriteOffset, stats.lastJournalWriteOffset);
            }
    ...
    {
        { "nativeGetPagerStats", "(Lcom/tencent/wcdb/database/SQLiteDebug$PagerStats;)V",
        ...
        { "nativeGetIOTraceStats", "(JLjava/util/ArrayList;)V", (void*) nativeGetIOTraceStats },
    };
...
#define GET_FIELD_ID(var, clazz, fieldName, fieldDescriptor) \
    ...
    static int register_wcdb_SQLiteDebug(JavaVM *vm, JNIEnv *env)
    {
        ...
        return jniRegisterNativeMethods(env, "com/tencent/wcdb/database/SQLiteDebug",

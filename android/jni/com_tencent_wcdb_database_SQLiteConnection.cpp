...
namespace wcdb {
    ...
    static void nativeSetKey(JNIEnv *env, jclass clazz, jlong connectionPtr, jbyteArray keyArr) {

        SQLiteConnection *connection = (SQLiteConnection *) (intptr_t) connectionPtr;
    ...
    static jlong nativeOpen(JNIEnv* env, jclass clazz, jstring pathStr, jint openFlags,
        jstring labelStr, jstring vfsNameStr) {
        int sqliteFlags;
    ...
    static void nativeClose(JNIEnv* env, jclass clazz, jlong connectionPtr) {
        ...
        if (connection) {
            ...
            if (err != SQLITE_OK) {
                throw_sqlite3_exception(env, connection->db, "Failed to close database.");
                return;
            }
            delete connection;
        }
    ...
    static void sqliteCustomFunctionCallback(sqlite3_context *context,
        ...
        if (argsArray) {
            for (int i = 0; i < argc; i++) {
                ...
                if (!arg) {
                    LOGW(LOG_TAG,
                         "NULL argument in custom_function_callback.  This should not happen.");
                } else {
    ...
    static void nativeResetStatement(JNIEnv* env, jclass clazz, jlong connectionPtr,
        jlong statementPtr, jboolean clearBindings) {
        sqlite3_stmt* statement = (sqlite3_stmt *) (intptr_t) statementPtr;
    ...
    static CopyRowResult copyRow(JNIEnv* env, CursorWindow* window,
        ...
        if (status) {
            LOGV(LOG_TAG, "Failed allocating fieldDir at startPos %d row %d, error=%d",
        ...
        for (int i = 0; i < numColumns; i++) {
            ...
            if (type == SQLITE_TEXT) {
                ...
                if (status) {
                    LOGW(LOG_TAG, "Failed allocating %zu bytes for text at %d,%d, error=%d",
                ...
                if (status) {
                    LOGW(LOG_TAG, "Failed allocating space for a long in column %d, error=%d",
                         i, status);
                    result = CPR_FULL;
                    break;
                }
                ...
                if (status) {
                    LOGW(LOG_TAG, "Failed allocating space for a double in column %d, error=%d",
                         i, status);
                    result = CPR_FULL;
                    break;
                }
                ...
                if (status) {
                    LOGW(LOG_TAG, "Failed allocating %zu bytes for blob at %d,%d, error=%d",
                ...
                if (status) {
                    LOGW(LOG_TAG, "Failed allocating space for a null in column %d, error=%d",
                         i, status);
                    result = CPR_FULL;
                    break;
                }
    ...
    static jlong nativeExecuteForCursorWindow(JNIEnv* env, jclass clazz,
        ...
        if (status) {
            char buf[128];
            snprintf(buf, sizeof(buf), "Failed to clear the cursor window, status=%d", status);
        ...
        if (status) {
            char buf[128];
            snprintf(buf, sizeof(buf), "Failed to set the cursor window column count to %d, status=%d",
                numColumns, status);
            throw_sqlite3_exception(env, connection->db, buf);
            return 0;
        }
    ...
    static int explain_callback(void *ud, int num_column, char **values, char **names)
    {
    ...
    static jstring nativeExplainQueryPlan(JNIEnv *env, jclass clazz, jlong connectionPtr, jstring sql)
    {
        SQLiteConnection *connection = (SQLiteConnection *) (intptr_t) connectionPtr;
        ...
        std::string out;
        char *errmsg;
        sqlite3_exec(connection->db, explain_sql.c_str(), explain_callback, &out, &errmsg);
        if (errmsg) {
            LOGE(LOG_TAG, "Failed to explain query plan for SQL '%s': %s", orig_sql, errmsg);
            sqlite3_free(errmsg);
    ...
    static jlong nativeGetDbLookaside(JNIEnv* env, jobject clazz, jlong connectionPtr) {
        SQLiteConnection *connection = (SQLiteConnection *) (intptr_t) connectionPtr;
    ...
    static jlong nativeGetSQLiteHandle(JNIEnv* env, jobject clazz, jlong connectionPtr) {
        SQLiteConnection *connection = (SQLiteConnection *) (intptr_t) connectionPtr;
    ...
    {
        { "nativeOpen", "(Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;)J", (void*) nativeOpen },
        { "nativeClose", "(J)V", (void*) nativeClose },
        { "nativeSetKey", "(J[B)V", (void*) nativeSetKey },
        { "nativeRegisterCustomFunction", "(JLcom/tencent/wcdb/database/SQLiteCustomFunction;)V",
    ...
    static int register_wcdb_SQLiteConnection(JavaVM *vm, JNIEnv *env)
    {
        ...
        int result = jniRegisterNativeMethods(env, "com/tencent/wcdb/database/SQLiteConnection",

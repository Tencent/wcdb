...
pub mod window_def_test {
    ...
    pub fn test() {
        ...
        window_def.partition_by(vec!["column1"]);
        WinqTool::winq_equal(&window_def, "(PARTITION BY column1)");

        let window_def = WindowDef::new();
        window_def.partition_by(vec!["column1", "column2"]);
        ...
        let window_def = WindowDef::new();
        window_def.partition_by(vec![&column1, &expression]);
        ...
        let window_def = WindowDef::new();
        window_def.order_by(&vec![&ordering_term]);
        WinqTool::winq_equal(&window_def, "(ORDER BY column1 ASC)");

        let window_def = WindowDef::new();
        window_def.frame_spec(FrameSpec::new().range().unbounded_preceding());
        ...
        let window_def = WindowDef::new();
        window_def
            .partition_by(vec![&column1, &expression])

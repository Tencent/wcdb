...
pub mod window_def_test {
    ...
    pub fn test() {
        ...
        let column1 = Column::new("column1").add_int(1);
        let window_def = WindowDef::new().partition_by_with_expression_convertible(&vec![&column1]);
        WinqTool::winq_equal(&window_def, "(PARTITION BY column1 + 1)");

        let column1 = Column::new("column1").add_int(1);
        let column2 = Column::new("column2");
        let expression = Expression::new_with_column(&column2);
        let window_def =
            WindowDef::new().partition_by_with_expression_convertible(&vec![&column1, &expression]);
        WinqTool::winq_equal(&window_def, "(PARTITION BY column1 + 1, column2)");

        let ordering_term: OrderingTerm = Column::new("column1").order(Order::Asc);
        ...
        let window_def = WindowDef::new()
            .partition_by_with_expression_convertible(&vec![&column1, &expression])

...
//         let expression =
//             Expression::new(LiteralValue::new(Option::from("abc")));
//         WinqTool::winq_equal(&expression, "'abc'");
//         let expression = Expression::new(LiteralValue::new_with_bool(false));
//         WinqTool::winq_equal(&expression, "FALSE");
//         let expression = Expression::new(LiteralValue::new_with_bool(true));
//         WinqTool::winq_equal(&expression, "TRUE");
//         let expression = Expression::new(&column);
//         WinqTool::winq_equal(&expression, "testColumn");
//         let expression = Expression::new(BindParameter::new_with_i32(1));
//         WinqTool::winq_equal(&expression, "?1");
//
//         let binding = StatementSelect::new();
//         let select = binding.select(&vec![&Column::new("testColumn")]);
...
//         WinqTool::winq_equal(&expression, "CAST(testColumn AS INTEGER)");
//
//         let column_row = Column::row_id().add(1).r#as("rowidAddOne");
//         WinqTool::winq_equal(&column_row, "rowid + 1 AS rowidAddOne");
//
//         let expression = Expression::r#case(None)
//             .when(&column.eq(1))
//             .then_with_string("a")
//             .when(&column.eq(2))
//             .then_with_string("b")
...
//         );
//
//         let expression = Expression::case(None)
//             .when(&column.eq("a"))
//             .then_with_i32(1)
//             .when_with_expression_convertible(&column.eq("b"))
//             .then_with_i32(2)
//             .else_with_i32(3);
//         WinqTool::winq_equal(
//             &expression,
...
//         let expression_left = Expression::new(&Column::new("left"));
//         let expression_right = Expression::new(&Column::new("right"));
//     }
//
//     #[test]

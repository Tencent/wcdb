...
impl JoinTest {
    ...
    fn select1(&self) -> StatementSelect {
        ...
        ret.select(&columns).from(vec!["testTable1"]);
        ret
    }
    ...
    fn select2(&self) -> StatementSelect {
        let column = Column::new("column2", None);
        let columns = vec![column];
        let ret = StatementSelect::new();
        ret.select(&columns).from(vec!["testTable2"]);
        ret
    }
...
pub mod join_test {
    ...
    pub fn test() {
        ...
        WinqTool::winq_equal(
            join_left.join(test_table_name2),
        ...
        WinqTool::winq_equal(
            join_left.join(&join_test.select2()),
            &*format!("{}{}{}", select1sql, " JOIN ", select2sql).to_string(),
        );

        let join_left = Join::new(test_table_name1);
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(join_left.with(test_table_name2), "testTable1, testTable2");
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.with(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.left_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.left_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.left_outer_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.left_outer_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.inner_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.inner_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.cross_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.cross_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.natural_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.natural_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.natural_left_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.natural_left_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.natural_left_outer_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.natural_left_outer_join(&join_test.select2()),
            &*format!(
                "{}{}{}",
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.natural_inner_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.natural_inner_join(&join_test.select2()),
        ...
        let join_right = Join::new(test_table_name2);
        WinqTool::winq_equal(
            join_left.natural_cross_join(test_table_name2),
        ...
        let select2sql = join_test.select2sql();
        WinqTool::winq_equal(
            join_left.natural_cross_join(&join_test.select2()),
        ...
        WinqTool::winq_equal(
            join_left.join(test_table_name2).on(&column1.eq(&column2)),
        ...
        WinqTool::winq_equal(
            join_left
                .join(&join_test.select2())
                .on(&column1.eq(&column2)),
            &*format!(
                "{}{}{}{}",
        ...
        WinqTool::winq_equal(
            join_left.join(test_table_name2).using(&column_vec),
        ...
        column_vec.push(column2);
        WinqTool::winq_equal(
            join_left.join(&join_test.select2()).using(&column_vec),
            &*format!(
                "{}{}{}{}",
    ...
    pub fn join_test1() {
        ...
        let _ = message_tag_table.insert_object(tag, Some(DbMessageTagTable::all_fields()));

        let mut tag = MessageTagTable::new();
        tag.tag_id = "10002".to_string();
        tag.tag_name = "10002_name".to_string();
        tag.create_time = 1790000001;
        let ret = message_tag_table.insert_object(tag, Some(DbMessageTagTable::all_fields()));
        ...
        // 连表查询
        let column_vec = vec![result_column1, result_column2, result_column3];
        let binding = StatementSelect::new();
        let tag_statement = binding.select(&column_vec).from(vec!["MessageTagTable"]);
        // conversation
        let column_vec: Vec<Column> = vec![
            Column::new("tag_id", None),
            Column::new("target_id", None),
            Column::new("category_id", None),
            Column::new("is_top", None),
        ];
        let binding = StatementSelect::new();
        let conversation_statement = binding
            .select(&column_vec)
            .from(vec!["ConversationTagTable"]);

        // 构建 join
        let column1 = Column::new("a_tag_id", None);
        let column2 = Column::new("tag_id", None);
        let join = Join::new(tag_statement);
        join.left_join(conversation_statement)
            .on(&column1.eq(&column2));

        // 构建查询需要的 StatementSelect
        let column_tag_id = Column::new("tag_id", None);
        column_tag_id.in_table("MessageTagTable");
        let select = StatementSelect::new();
        select.select(&vec![Column::all()]).from(&vec![join]);

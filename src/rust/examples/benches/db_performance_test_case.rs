...
fn insert_data_performance(
    ...
    let _ = table.insert_objects(vec, Some(DbFriendProfileTable::all_fields()));
}

fn select_data_performance(database: &Database, size: i64) {
    let column_vec: Vec<Column> = vec![
        Column::new("user_id", None),
        Column::new("remark", None),
        Column::new("friend_type", None),
        Column::new("is_top", None),
        Column::new("add_time", None),
    ];
    let binding = StatementSelect::new();
    let condition = Column::new("add_time", None).gt(1);
    let statement = binding
        .select(&column_vec)
        .from(vec!["FriendProfileTable"])
        .where_(&condition)
        .limit(size);
    // SELECT user_id, remark, friend_type, is_top, add_time FROM FriendProfileTable WHERE add_time > 1 LIMIT 1
    let _: WCDBResult<Vec<Vec<Value>>> = database.get_all_rows_from_statement(statement);
}

fn update_data_performance(database: &Database, size: i64) {
    let column = Column::new("is_top", None);
    let column_vec: Vec<&Column> = vec![&column];
    let statement = StatementUpdate::new();
    let condition = Column::new("is_top", None)
        .not_eq(true)
        .and(&Column::new("add_time", None).gt(1));
    statement
    ...
    let _ = database.execute(&statement);
}
...
fn index_data_performance(database: &Database) {
    ...
    let column1 = Column::new("add_time", None);
